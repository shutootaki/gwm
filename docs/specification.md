...
- **対話的 UI の優先:** 引数なしで実行された場合、インクリメンタル検索と選択 UI を提供し、ユーザーの記憶やタイピングの負担を軽減する。
...
- **構文:** `gwm remove [query]`
- **引数:**
  - `query` (任意): 削除したい worktree のブランチ名を指定する。検索の初期クエリとして使用される。
- **オプション:**
...
- **引数:**
  - `query` (任意): 移動・オープンしたい worktree のブランチ名を指定。検索の初期クエリとして使用。
- **オプション:**
...
- **CLI ライブラリ: Ink**
  - React コンポーネントを用いて CLI の UI を構築するためのライブラリ。これにより、ターミナル上でもリッチでインタラクティブなユーザー体験（例: インタラクティブなリスト選択）を提供できる。
