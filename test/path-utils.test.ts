...
describe('Path Generation and Normalization Tests', () => {
  ...
  describe('normalizeWorktreeBranchName', () => {
    ...
    it('should convert slashes to hyphens in branch names', () => {
      expect(normalizeWorktreeBranchName('feature/user-auth')).toBe(
        'feature-user-auth'
      );
      expect(normalizeWorktreeBranchName('bugfix/login-issue')).toBe(
        'bugfix-login-issue'
      );
      expect(normalizeWorktreeBranchName('release/v1.2.3')).toBe(
        'release-v1.2.3'
      );
    });
    ...
    it('should handle multiple slashes in branch names', () => {
      expect(normalizeWorktreeBranchName('team/user/feature/auth')).toBe(
        'team-user-feature-auth'
      );
      expect(normalizeWorktreeBranchName('feat/api/v2/endpoints')).toBe(
    ...
    it('should return branch name as-is when no slashes present', () => {
      ...
      expect(normalizeWorktreeBranchName('hotfix-urgent')).toBe(
        'hotfix-urgent'
      );
    });
  ...
  describe('getRepositoryName', () => {
    ...
    it('should extract repository name from path', () => {
      ...
      expect(getRepositoryName('/home/user/code/awesome-project')).toBe(
        'awesome-project'
      );
  ...
  describe('expandTildePath', () => {
    ...
    it('should expand tilde notation to home directory', () => {
      ...
      expect(expandTildePath('~/dev/projects')).toBe(
        '/Users/test/dev/projects'
      );
      expect(expandTildePath('~/.config/gwm')).toBe('/Users/test/.config/gwm');
    });
  ...
  describe('Path Validation', () => {
    ...
    it('should handle very long branch names', () => {
      const repositoryPath = '/Users/test/projects/my-app';
      const veryLongBranchName =
        'feature/' + 'a'.repeat(100) + '/very/long/nested/branch/name';
  ...
  describe('Configuration Integration', () => {
    ...
    it('should respect configuration changes', () => {
      // 初期設定
      let generatedPath = generateWorktreePath(
