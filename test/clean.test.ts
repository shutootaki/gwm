...
describe('wtm clean command integration tests', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should detect and clean merged worktrees', async () => {
    ...
    const prunableWorktrees = mockWorktrees.filter(w => w.status === 'PRUNABLE');
    
    expect(prunableWorktrees).toHaveLength(1);
    expect(prunableWorktrees[0].path).toBe('/Users/test/worktrees/project/merged-feature');
    ...
    expect(mockRemoveWorktree).toHaveBeenCalledWith('/Users/test/worktrees/project/merged-feature');
  });
  ...
  it('should detect worktrees with deleted remote branches', async () => {
    ...
    mockExecSync.mockImplementation((command) => {
      if (command?.includes('git show-ref --verify --quiet refs/remotes/origin/deleted-remote')) {
  ...
  it('should auto-remove all prunable worktrees with --yes flag', async () => {
    ...
    expect(mockRemoveWorktree).toHaveBeenCalledWith('/Users/test/worktrees/project/merged-1');
    expect(mockRemoveWorktree).toHaveBeenCalledWith('/Users/test/worktrees/project/merged-2');
  });
  ...
  it('should allow selective removal in interactive mode', async () => {
    ...
    expect(mockRemoveWorktree).toHaveBeenCalledWith('/Users/test/worktrees/project/merged-1');
    expect(mockRemoveWorktree).not.toHaveBeenCalledWith('/Users/test/worktrees/project/merged-2');
  });
  ...
  it('should check merge status against all configured main branches', async () => {
    ...
    mockExecSync.mockImplementation((command) => {
      if (command?.includes('git merge-base --is-ancestor refs/heads/feature-merged-to-develop refs/heads/develop')) {
        return ''; // マージ済み
      }
      if (command?.includes('git merge-base --is-ancestor refs/heads/feature-merged-to-develop refs/heads/main')) {
        throw new Error('Not merged to main');
      }
      if (command?.includes('git merge-base --is-ancestor refs/heads/feature-merged-to-develop refs/heads/master')) {
    ...
    expect(prunableWorktrees).toHaveLength(1);
    expect(prunableWorktrees[0].branch).toBe('refs/heads/feature-merged-to-develop');
  });
  ...
  it('should exclude main worktree from cleanup candidates', async () => {
    ...
    const prunableWorktrees = mockWorktrees.filter(w => w.status === 'PRUNABLE' && !w.isMain);
  ...
  it('should handle locked worktrees appropriately', async () => {
    ...
    expect(lockedWorktrees[0].path).toBe('/Users/test/worktrees/project/locked-but-merged');
  });
});